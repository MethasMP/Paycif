...
class SecurityRepositoryImpl implements SecurityRepository {
  ...
  Future<void> verifyPin(String pin) async {
    ...
    try {
      final localHash = await _secureStorage.read(_kLocalPinHashKey);
      final localSaltB64 = await _secureStorage.read(_kLocalPinSaltKey);

      if (localHash != null && localSaltB64 != null) {
        final localSalt = base64Decode(localSaltB64);
        final isValid = await _cryptoService.verifyPinHashWithSalt(
          pin,
          localHash,
          localSalt,
        );
        if (isValid) {
          ...
          debugPrint('✅ [Verify] Optimistic Local Check PASSED');
        } else {
