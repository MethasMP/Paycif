...
type SignatureService struct {
	...
	udsPath string
	cache   sync.Map
	client  *http.Client // Reusable single HTTP client across requests
}
...
func NewSignatureService(db *sql.DB, udsPath string) *SignatureService {
	...
	// Optimized pre-allocated http.Client with connection pooling enabled over UDS
	client := &http.Client{
...
func (s *SignatureService) VerifySignature(ctx context.Context, publicKeyB64, signatureB64, message string) (bool, error) {
	...
	// Reuse pre-allocated thread-safe client instance
