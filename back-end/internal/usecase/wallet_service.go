...
func (s *WalletService) GetExchangeRate(ctx context.Context, fromCurr, toCurr string) (*ExchangeRateResponse, error) {
	// Performance: Using string concatenation instead of fmt.Sprintf for hot-path cache keys
...
func (s *WalletService) ProcessPayment(ctx context.Context, userID uuid.UUID, amount float64, merchant string, referenceID string) error {
	...
	// 1 & 2. Record Transaction with Atomic Idempotency Check
	// Performance: Combined SELECT EXISTS + INSERT into a single atomic operation using ON CONFLICT DO NOTHING.
	// This reduces database round-trips and shortens transaction duration.
	newTxID := uuid.New()
