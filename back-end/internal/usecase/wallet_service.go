...
func (s *WalletService) ProcessPayment(ctx context.Context, userID uuid.UUID, amount float64, merchant string, referenceID string) error {
	...
	// Record Transaction with ON CONFLICT DO NOTHING for atomic idempotency
	newTxID := uuid.New()
	...
	if rowsAffected == 0 {
		logger.WithContext(ctx).Info("Payment already processed (idempotent conflict)", "reference_id", referenceID)
		return nil
	}
