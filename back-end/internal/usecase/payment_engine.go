...
type PaymentProvider interface {
	...
	Payout(ctx context.Context, amount int64, currency string, recipientID string, recipientName string, reference string, providerTxID string, customerID string) (*PayoutResult, error)
}
...
func (e *PaymentEngine) ExecutePayout(ctx context.Context, providerName string, amount int64, currency string, recipientID string, recipientName string, reference string, providerTxID string, customerID string) (*PayoutResult, error) {
	e.mu.RLock()
	name := providerName
	if name == "" {
	...
	return provider.Payout(ctx, amount, currency, recipientID, recipientName, reference, providerTxID, customerID)
}

// GetProvider returns a registered provider by name. Thread-safe.
func (e *PaymentEngine) GetProvider(name string) (PaymentProvider, bool) {
	e.mu.RLock()
	defer e.mu.RUnlock()
...
func (w *WiseProvider) Payout(ctx context.Context, amount int64, currency string, recipientID string, recipientName string, reference string, providerTxID string, customerID string) (*PayoutResult, error) {
	// Mock implementation
