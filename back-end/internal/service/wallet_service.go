...
func (s *WalletService) GetExchangeRate(ctx context.Context, fromCurr, toCurr string) (*ExchangeRateResponse, error) {
	// Optimization: Normalize currency codes to uppercase before cache key generation and DB query.
	// This ensures consistent cache hits and prevents duplicate entries.
...
func (s *WalletService) ProcessPayment(ctx context.Context, userID uuid.UUID, amount float64, merchant string, referenceID string) error {
	...
	// Optimization: Unified Idempotency & Insert
	// Using ON CONFLICT DO NOTHING to reduce RTT and serialization conflicts in SERIALIZABLE isolation.
	newTxID := uuid.New()
...
func (s *WalletService) PayoutToPromptPay(ctx context.Context, req PayoutRequest) (*PayoutResponse, error) {
	...
	// Optimization: Filter directly on ledger_entries.created_at to avoid redundant JOIN with transactions
	// and leverage idx_ledger_entries_profile_created composite index.
