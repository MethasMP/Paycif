...
func VerifyPassportNfcSignature(payload NfcPassportPayload) (*PassportIdentity, error) {
	...
	// 1. Extract Identity from DG1 (The source of truth)
	// Parse DG1 only once upfront and reuse the result to avoid double parsing.
...
func parseDG1(data []byte) (*PassportIdentity, error) {
	// Simple scanner for the MRZ string within DG1 (using byte scanning to reduce string allocations)
	mrzIdx := -1
	...
	// Extract the substrings directly from mrzBytes to minimize heap allocations
	docNum := strings.TrimRight(string(mrzBytes[44:53]), "<")
	nationality := string(mrzBytes[54:57])
	dob := string(mrzBytes[57:63])
