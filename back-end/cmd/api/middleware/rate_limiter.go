...
func RateLimiterMiddleware() gin.HandlerFunc {
	...
	return func(c *gin.Context) {
		...
		currentMinute := time.Now().Unix() / 60
		// Optimize: Use direct string concatenation instead of fmt.Sprintf to eliminate reflection and heap allocations on hot path
...
// Inc atomically increments counter to avoid mutex lock overhead on hot paths
