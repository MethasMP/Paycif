	...
	if exists && atomic.LoadInt32(&entry.Hydrated) == 1 && !entry.IsStale(c.maxAge) {
		...
		for {
			...
			if currentTotal+amount > maxDailySatang {
				...
				// Bolt optimization: Replace fmt.Sprintf with strconv.FormatFloat and string concatenation
				return false, remaining, "Daily limit exceeded. Remaining: " + strconv.FormatFloat(remF64, 'f', 2, 64), nil
			}
...
func (s *AccountingService) Transfer(ctx context.Context, in *pb.TransferRequest) (*pb.TransferResponse, error) {
	...
	if resp.Success && s.rdb != nil {
		...
		amount := in.Amount
		go func() {
			// Bolt optimization: Replace fmt.Sprintf with string concatenation and strconv.FormatInt
